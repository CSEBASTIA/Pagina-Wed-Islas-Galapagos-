...
// Match por tour_id (prioritario) o por nombre parcial como fallback
function matchReview(r, t) {
    // Match por ID numérico (ignorar 0 y null)
    if (r.tour_id != null && Number(r.tour_id) > 0 && Number(r.tour_id) === Number(t.id)) {
        return true;
    }
    // Fallback: match por nombre (sin prefijo "tour ")
    const a = String(r.tour_name || '').toLowerCase().replace(/^tour\s+/, '').trim();
    const b = String(t.title || '').toLowerCase().replace(/^tour\s+/, '').trim();
...
async mount(){
  ...
  try{
    const [tours, reviews] = await Promise.all([
      ...
      typeof ReviewsService!=='undefined' ? ReviewsService.getAll().catch(e => {
          console.error('Blog: Error cargando reseñas:', e);
          return [];
      }) : Promise.resolve([]),
    ]);

    console.log(`Blog: ${(tours||[]).length} tours, ${(reviews||[]).length} reseñas cargadas`);
    ...
    console.error('Blog: Error general:', e);
    if(err){err.textContent='No se pudieron cargar datos del blog: ' + e.message;err.classList.remove('hidden');}
  }
}
};
})();
